...
class Evaluator(object):
    def __init__(self, dataset, class_num, norm_mean, norm_std, network, multi_scales, 
        ...
        self.config = config
        self.output = nn.Conv2d(in_channels=24, out_channels=3, kernel_size=1, bias=False)
    ...
    def sliding_eval_rgbX(self, img, modal_x, lf, crop_size, stride_rate, device=None):
        ...
        for s in self.multi_scales:
            ...
            if len(next(iter(lf.values())).shape) == 2:
                lf_scale = {key: cv2.resize(img, None, fx=s, fy=s, interpolation=cv2.INTER_NEAREST) for key, img in
                            lf.items()}
            else:
                lf_scale = {key: cv2.resize(img, None, fx=s, fy=s, interpolation=cv2.INTER_LINEAR) for key, img in
                            lf.items()}

            new_rows, new_cols, _ = img_scale.shape
            processed_pred += self.scale_process_rgbX(img_scale, modal_x_scale, lf_scale, (ori_rows, ori_cols), crop_size, stride_rate, device)

        pred = processed_pred.argmax(2)

        return pred

    def scale_process_rgbX(self, img, modal_x, lf, ori_shape, crop_size, stride_rate, device=None):
        new_rows, new_cols, c = img.shape
        long_size = new_cols if new_cols > new_rows else new_rows

        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            input_data, input_modal_x, input_lf, margin = self.process_image_rgbX(img, modal_x, lf, crop_size)
            score = self.val_func_process_rgbX(input_data, input_modal_x, input_lf, device)
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    input_data, input_modal_x, input_lf, tmargin = self.process_image_rgbX(
                        img_sub, modal_x_sub, lf_sub, crop_size)

                    temp_score = self.val_func_process_rgbX(input_data, input_modal_x, input_lf, device)
    ...
    def val_func_process_rgbX(self, input_data, input_modal_x, input_lf, device=None):
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():
                score = self.val_func(input_data, input_modal_x, input_lf)
                score = score[0]
                if self.is_flip:
                    input_data = input_data.flip(-1)
                    input_modal_x = input_modal_x.flip(-1)
                    input_lf = {key: img.flip(-1) for key, img in input_lf.items()}
                    score_flip = self.val_func(input_data, input_modal_x, input_lf)
                    score_flip = score_flip[0]
                    score += score_flip.flip(-1)
                score = torch.exp(score)
    ...
    def process_image_rgbX(self, img, modal_x, lf, crop_size=None):
        p_img = img
        p_modal_x = modal_x
        p_lf = lf
        ...
        if crop_size is not None:
            ...
            return p_img, p_modal_x, p_lf, margin
