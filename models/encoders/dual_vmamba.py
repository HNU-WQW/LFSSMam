...
class RGBXTransformer(nn.Module):
    ...
    def forward_features(self, x_rgb, x_e, x_lf):
        """
        x_rgb: B x C x H x W
        """
        B = x_rgb.shape[0]
        ...
        for i in range(4):
            ...
            cam = True
            if cma and cam:
                cross_rgb, cross_x, cross_lf = self.lf_cross_mamba[i](out_rgb.permute(0, 2, 3, 1).contiguous(),
                                                                   out_x.permute(0, 2, 3, 1).contiguous(),
                                                                   out_lf.permute(0, 2, 3, 1).contiguous()) # B x H x W x C
                x_fuse = self.lf_concat_mamba[i](cross_rgb, cross_x, cross_lf).permute(0, 3, 1, 2).contiguous()
            elif cam and not cma:
                x_fuse = self.lf_concat_mamba[i](out_rgb.permute(0, 2, 3, 1).contiguous(),
                                                    out_x.permute(0, 2, 3, 1).contiguous(),
                                                    out_lf.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous()
            elif not cam and not cma:
                x_fuse = (out_rgb + out_x + out_lf)
            outs_fused.append(x_fuse)        
        return outs_fused

    def forward(self, x_rgb, x_e, x_lfs):
        x_lfs_list = [x_lfs[key] for key in x_lfs]  # 获取所有的张量
        # 将所有张量沿着维度 dim=1 进行拼接
        x_lf = torch.cat(x_lfs_list, dim=1)
        out = self.forward_features(x_rgb, x_e, x_lf)
        return out
