...
if True:
    ...
    class CrossScan_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x_rgb: torch.Tensor, x_e: torch.Tensor, x_lf: torch.Tensor):
            # B, C, H, W -> B, 3, C, 2 * H * W
            B, C, H, W = x_rgb.shape
            ctx.shape = (B, C, H, W)
            xs_fuse = x_rgb.new_empty((B, 3, C, 3 * H * W))
            xs_fuse[:, 0] = torch.cat([x_rgb.flatten(2, 3), x_e.flatten(2, 3), x_lf.flatten(2, 3)], dim=2)
        ...
        def backward(ctx, ys: torch.Tensor):
            ...
            # get B, d, H*W
            return ys[:, :, 0:H * W].view(B, -1, H, W), ys[:, :, H * W:2 * H * W].view(B, -1, H, W), ys[:, :, 2 * H * W:3 * H * W].view(B, -1, H, W)


    class CrossScan8_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x_1_1: torch.Tensor, x_1_9: torch.Tensor, x_3_3: torch.Tensor, x_3_7: torch.Tensor,
                    x_7_3: torch.Tensor, x_7_7: torch.Tensor, x_9_1: torch.Tensor, x_9_9: torch.Tensor):
            # B, C, H, W -> B, 3, C, 2 * H * W
            B, C, H, W = x_1_1.shape
            ctx.shape = (B, C, H, W)
            xs_fuse = x_1_1.new_empty((B, 8, C, 8 * H * W))
            xs_fuse[:, 0] = torch.cat([x_1_1.flatten(2, 3), x_1_9.flatten(2, 3), x_3_3.flatten(2, 3), x_3_7.flatten(2, 3),
                                       x_7_3.flatten(2, 3), x_7_7.flatten(2, 3), x_9_1.flatten(2, 3), x_9_9.flatten(2, 3)], dim=2)
            xs_fuse[:, 1] = torch.flip(xs_fuse[:, 0], dims=[-1])
        ...
        def backward(ctx, ys: torch.Tensor):
            ...
            ys = ys[:, 0] + ys[:, 1].flip(dims=[-1])  # B, d, 2 * H * W
            # get B, d, H*W
            return ys[:, :, 0:H * W].view(B, -1, H, W), ys[:, :, H * W:2 * H * W].view(B, -1, H, W), ys[:, :,
                                                                                                     2 * H * W:3 * H * W].view(
                B, -1, H, W)
    ...
    class CrossMerge_multimodal(torch.autograd.Function):
        ...
        def forward(ctx, ys: torch.Tensor):
            ...
            # y = ys[:, :, 0:L//2] + ys[:, :, L//2:L]
            return ys[:, :, 0:L//3], ys[:, :, L//3:2*L//3], ys[:, :, 2*L//3:L]

        @staticmethod
        def backward(ctx, x1: torch.Tensor, x2: torch.Tensor, x3: torch.Tensor):
            # B, D, L = x.shape
            # out: (b, k, d, l)
            # H, W = ctx.shape
            B, C, L = x1.shape
            xs = x1.new_empty((B, 3, C, 3*L))
            xs[:, 0] = torch.cat([x1, x2, x3], dim=2)
            xs[:, 1] = torch.flip(xs[:, 0], dims=[-1])
            xs = xs.view(B, 3, C, 3*L)
            return xs, None, None

    class CrossMerge8_multimodal(torch.autograd.Function):
            @staticmethod
            def forward(ctx, ys: torch.Tensor):
                B, K, D, L = ys.shape
                # ctx.shape = (H, W)
                ...
                ys = ys[:, 0] + ys[:, 1].flip(dims=[-1])  # B, d, 2 * H * W, broadcast
                # y = ys[:, :, 0:L//2] + ys[:, :, L//2:L]
                return ys[:, :, 0:L // 8], ys[:, :, L // 8:2 * L // 8], ys[:, :, 2 * L // 8:3 * L // 8], ys[:, :, 3 * L // 8:4 * L // 8], ys[:, :, 4 * L // 8:5 * L // 8], ys[:, :, 5 * L // 8:6 * L // 8], ys[:, :, 6 * L // 8:7 * L // 8], ys[:, :, 7 * L // L]

            @staticmethod
            def backward(ctx, x1: torch.Tensor, x2: torch.Tensor, x3: torch.Tensor, x4: torch.Tensor,
                         x5: torch.Tensor, x6: torch.Tensor, x7: torch.Tensor, x8: torch.Tensor):
                # B, D, L = x.shape
                # out: (b, k, d, l)
                # H, W = ctx.shape
                B, C, L = x1.shape
                xs = x1.new_empty((B, 8, C, 8 * L))
                xs[:, 0] = torch.cat([x1, x2, x3, x4, x5, x6, x7, x8], dim=2)
                xs[:, 1] = torch.flip(xs[:, 0], dims=[-1])
                xs = xs.view(B, 8, C, 8 * L)
                return xs, None, None
    ...
    def cross_selective_scan_multimodal_k2(
        ...
        x_fuse = CrossScan_multimodal.apply(x_rgb, x_e, x_lf) # B, C, H, W -> B, 3, C, 3 * H * W
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", x_fuse, x_proj_weight)
        #x_fuse.shape:[1, 3, 256, 57600],x_proj_weight torch.Size([3, 20, 256])
        if x_proj_bias is not None:
            x_dbl = x_dbl + x_proj_bias.view(1, K, -1, 1)
        dts, Bs, Cs = torch.split(x_dbl, [R, N, N], dim=2)
        dts = torch.einsum("b k r l, k d r -> b k d l", dts, dt_projs_weight)
...
class LFConMB_SS2D(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.in_proj_lf = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)

        # self.ca = Channel_Attention(768)
        ...
        self.out_proj = nn.Linear(self.d_inner*3, self.d_model, bias=bias, **factory_kwargs)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else nn.Identity()
    ...
    def forward_corev2_multimodal(self, x_rgb: torch.Tensor, x_e: torch.Tensor, x_lf: torch.Tensor, nrows=-1):
        return cross_selective_scan_multimodal_k2(
            x_rgb, x_e, x_lf, self.x_proj_weight, None, self.dt_projs_weight, self.dt_projs_bias,
            self.A_logs, self.Ds, getattr(self, "out_norm1", None), getattr(self, "out_norm2", None), self.softmax_version, 
            nrows=nrows,
        )

    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor, x_lf: torch.Tensor):
        x_rgb = self.in_proj(x_rgb)
        x_e = self.in_proj_modalx(x_e)
        x_lf = self.in_proj_lf(x_lf)
        if self.d_conv > 1:
            x_rgb_trans = x_rgb.permute(0, 3, 1, 2).contiguous()
            x_e_trans = x_e.permute(0, 3, 1, 2).contiguous()
            x_lf_trans = x_lf.permute(0, 3, 1, 2).contiguous()
            x_rgb_conv = self.act(self.conv2d(x_rgb_trans)) # (b, d, h, w)
            x_e_conv = self.act(self.conv2d_modalx(x_e_trans)) # (b, d, h, w)
            x_lf_conv = self.act(self.conv2d_lf(x_lf_trans))  # (b, d, h, w)
            #cncat ss shi xian guo cheng
            y_rgb, y_e, y_lf = self.forward_corev2_multimodal(x_rgb_conv, x_e_conv, x_lf_conv) # b, d, h, w -> b, h, w, d
            ...
            x_lf_squeeze = self.avg_pool(x_lf_trans).view(b, d)
            ...
            x_lf_exitation = self.fc3(x_lf_squeeze).view(b, d, 1, 1).permute(0, 2, 3, 1).contiguous()
            y_rgb = y_rgb * x_e_exitation * 0.5 + y_rgb * x_lf_exitation * 0.5
            y_e = y_e * x_rgb_exitation
            y_lf = y_lf * x_rgb_exitation
            y = torch.concat([y_rgb, y_e, y_lf], dim=-1)
...
# =====================================================
class LF_Cross_Mamba_Attention_SSM(nn.Module):
    def __init__(
            self,
            # basic dims ===========
            d_model=96,
        ...
        self.x_proj_3 = nn.Linear(self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs)
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor, x_lf: torch.Tensor):
        ...
        y_lf = selective_scan(
            x_lf, dt_lf,
            A_lf, B_lf, C_rgb, self.D_3.float(),
            delta_bias=self.dt_proj_3.bias.float(),
            delta_softplus=True,
        )
        # assert out_y.dtype == torch.float
        ...
        y_lf = rearrange(y_lf, "b d l -> b l d")
        y_lf = self.out_norm_3(y_lf)
        return y_rgb, y_e, y_lf


# =====================================================
class LFCrossMambaFusion_SS2D_SSM(nn.Module):
    '''
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.out_proj_lf = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
        ...
        self.dropout_lf = nn.Dropout(dropout) if dropout > 0. else nn.Identity()
        
        self.CMA_ssm = LF_Cross_Mamba_Attention_SSM(
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor, x_lf: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            x_lf_conv = self.act(self.conv2d(x_lf_trans))  # (b, d, h, w)
            x_rgb_conv = rearrange(x_rgb_conv, "b d h w -> b (h w) d")
            x_e_conv = rearrange(x_e_conv, "b d h w -> b (h w) d")
            x_lf_conv = rearrange(x_lf_conv, "b d h w -> b (h w) d")
            y_rgb, y_e, y_lf = self.CMA_ssm(x_rgb_conv, x_e_conv, x_lf_conv)
            # to b, d, h, w
            y_rgb = y_rgb.view(B, H, W, -1)
            y_e = y_e.view(B, H, W, -1)
            y_lf = y_lf.view(B, H, W, -1)


        out_rgb = self.dropout_rgb(self.out_proj_rgb(y_rgb))
        out_e = self.dropout_e(self.out_proj_e(y_e))
        out_lf = self.dropout_lf(self.out_proj_lf(y_lf))

        return out_rgb, out_e, out_lf
...
class LFCrossMambaFusionBlock(nn.Module):
    '''
    LF Cross Mamba Fusion (LFCroMB) fusion, with 2d SSM
    '''
    def __init__(
        self,
        hidden_dim: int = 0,
        ...
        **kwargs,
        ...
        self.use_checkpoint = use_checkpoint
        # self.norm = norm_layer(hidden_dim)
        self.op = LFCrossMambaFusion_SS2D_SSM(
            d_model=hidden_dim, 
            dropout=attn_drop_rate, 
            d_state=d_state, 
            ssm_ratio=ssm_ratio, 
    ...
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor, x_lf: torch.Tensor):
        x_rgb_cross, x_e_cross, x_lf_cross = self.op(x_rgb, x_e, x_lf)
        x_rgb = x_rgb + self.drop_path1(x_rgb_cross)
        x_e = x_e + self.drop_path2(x_e_cross)
        x_lf = x_lf + self.drop_path3(x_lf_cross)
        # x_lf = self.drop_path3(x_lf_cross)
        return x_rgb, x_e, x_lf
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor, x_lf: torch.Tensor):
        ...
        '''
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, x_rgb, x_e, x_lf)
        else:
            return self._forward(x_rgb, x_e, x_lf)


class LFConcatMambaFusionBlock(nn.Module):
    '''
    ...
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor, x_lf: torch.Tensor):
        x = x_rgb + x_e + x_lf + self.drop_path(self.op(x_rgb, x_e, x_lf))
        # x = x_rgb + x_e + self.drop_path(self.op(x_rgb, x_e, x_lf))
        if self.mlp_branch:
            x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN
        return x
