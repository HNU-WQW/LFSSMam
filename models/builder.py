...
class EncoderDecoder(nn.Module):
    ...
    def encode_decode(self, rgb, modal_x, lf):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""

        if not self.deep_supervision:
            orisize = rgb.shape
            x = self.backbone(rgb, modal_x, lf)
    ...
    def forward(self, rgb, modal_x, lf, label=None):

        if not self.deep_supervision:
            if self.aux_head:
                out, aux_fm = self.encode_decode(rgb, modal_x, lf)
            else:
                out = self.encode_decode(rgb, modal_x, lf)
            ...
            x_last, x_output_0, x_output_1, x_output_2 = self.encode_decode(rgb, modal_x, lf)
            if label is not None:
    ...
    def flops(self, shape=(3, 480, 640)):
        ...
        # 定义输入
        input_tensor_rgb = torch.randn((1, *shape), device=next(model.parameters()).device)
        input_tensor_x = torch.randn((1, *shape), device=next(model.parameters()).device)
        input_dict = {
        ...
        # 将输入组合成一个元组
        input = (input_tensor_rgb, input_tensor_x, input_dict)
