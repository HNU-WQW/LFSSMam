...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device, config):
        self.output = nn.Conv2d(in_channels=12, out_channels=3, kernel_size=1, bias=False)
        img = data['data']
        label = data['label']
        ...
        pred = self.sliding_eval_rgbX(img, modal_x, lf, config.eval_crop_size, config.eval_stride_rate, device)
